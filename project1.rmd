...
The assignment names are quite long so I'll shorten the ISLRv2 full name to the
abbreviation.
...
nonz_durations$assignment %>% table() %>% knitr::kable()
```
...
To see how the type of assignment affects duration, I'll add `assignment_type`
and `assignment_source` columns.
...
nonz_durations %>%
    ...
    mutate(assignment_source =
           case_when(
                     grepl("ISLRv2", assignment) ~ "ISLRv2",
                     grepl("Experimental Design for Biologists", assignment) ~ "EDB",
                     grepl("RStudio", ignore.case = TRUE, assignment) ~ "Software Carpentry")) -> nonz_durations


nonz_durations %>% head() %>% knitr::kable()
```

```{r, fig.width=10, fig.height=10}
nonz_durations %>%
    ggplot(aes(x = reorder(assignment_type, duration), y = duration)) +
    geom_boxplot() +
    geom_jitter(aes(color = ID)) +
    xlab("Assignment Type") +
    facet_grid(cols = vars(assignment_source)) +
    theme(legend.position = "bottom",
          axis.text.y = element_text(size = 12))
```
